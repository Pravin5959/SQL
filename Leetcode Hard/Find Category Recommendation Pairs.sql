...
WITH buyers AS (SELECT category,
                       COUNT(DISTINCT user_id) AS total_buyers,
                       ARRAY_AGG(DISTINCT user_id) AS buyers_id
...
SELECT a.category AS category1,
       ...
       (SELECT array_length(array_agg(DISTINCT val),1)
        FROM (
                SELECT UNNEST(c.buyers_id) AS val
                INTERSECT
                SELECT UNNEST(d.buyers_id) AS val
            ) AS common_vals
        ) AS customer_count
FROM ProductInfo a
CROSS JOIN ProductInfo b
LEFT JOIN buyers c
ON a.category = c.category
LEFT JOIN buyers d
ON b.category = d.category
WHERE a.category < b.category
      AND c.total_buyers >= 3
      AND d.total_buyers >= 3
GROUP BY a.category,
         b.category,
         c.buyers_id,
         d.buyers_id
HAVING (SELECT array_length(array_agg(DISTINCT val),1)
            FROM (
                SELECT UNNEST(c.buyers_id) AS val
                INTERSECT
                SELECT UNNEST(d.buyers_id) AS val
