...
SELECT a.patient_id,
       b.patient_name,
       b.age,
       (MIN(CASE WHEN result = 'Negative' THEN test_date END) - 
        MIN(CASE WHEN result = 'Positive' THEN test_date END)) AS recovery_time
FROM
    (SELECT patient_id,
        test_date,
        result,
        MIN(CASE WHEN result = 'Positive' THEN test_date END) OVER(PARTITION BY patient_id) AS first_positive_date
    FROM covid_tests) a
INNER JOIN patients b
ON a.patient_id = b.patient_id
WHERE test_date >= first_positive_date
GROUP BY a.patient_id,
         b.patient_name,
         b.age
HAVING (MIN(CASE WHEN result = 'Negative' THEN test_date END) - 
        MIN(CASE WHEN result = 'Positive' THEN test_date END)) > 0
