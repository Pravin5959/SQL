...
SELECT e.employee_id,
       name,
       rating - second_last_rating AS improvement_score
FROM
    (SELECT employee_id,
           rating,
           LEAD(rating) OVER(PARTITION BY employee_id ORDER BY review_date DESC) AS last_rating, -- last rating
           LEAD(rating, 2) OVER(PARTITION BY employee_id ORDER BY review_date DESC) AS second_last_rating, -- second last rating
           ROW_NUMBER() OVER(PARTITION BY employee_id ORDER BY review_date DESC) AS latest_review -- latest review record
    FROM performance_reviews) r
INNER JOIN employees e
ON r.employee_id = e.employee_id
WHERE rating > last_rating
      AND last_rating > second_last_rating -- mono increasing rated employee
      AND latest_review = 1
