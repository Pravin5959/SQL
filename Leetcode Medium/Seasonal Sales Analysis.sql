...
SELECT season,
       ...
       total_revenue
FROM
    (SELECT CASE WHEN TRIM(TO_CHAR(sale_date, 'Month')) 
                    IN ('December','January','February') 
                        THEN 'Winter'
                WHEN TRIM(TO_CHAR(sale_date, 'Month'))
                    IN ('March','April','May')
                        THEN 'Spring'
                WHEN TRIM(TO_CHAR(sale_date, 'Month'))
                    IN ('June','July','August')
                        THEN 'Summer'
                ELSE 'Fall'
            END AS season, -- Creation of season column
            category,
            SUM(quantity) AS total_quantity,
            SUM(quantity * price) AS total_revenue,
            ROW_NUMBER() OVER(PARTITION BY CASE WHEN TRIM(TO_CHAR(sale_date, 'Month')) 
                    IN ('December','January','February') 
                        THEN 'Winter'
                WHEN TRIM(TO_CHAR(sale_date, 'Month'))
                    IN ('March','April','May')
                ...
                WHEN TRIM(TO_CHAR(sale_date, 'Month'))
                    IN ('June','July','August')
                        THEN 'Summer'
                ELSE 'Fall'
            END ORDER BY SUM(quantity) DESC, SUM(quantity * price) DESC) AS product_rnk -- Ranking based on max quantity and total price
    FROM sales s
    INNER JOIN products p
    ON s.product_id = p.product_id
    GROUP BY CASE WHEN TRIM(TO_CHAR(sale_date, 'Month')) 
                    IN ('December','January','February') 
                        THEN 'Winter'
                WHEN TRIM(TO_CHAR(sale_date, 'Month'))
                    IN ('March','April','May')
